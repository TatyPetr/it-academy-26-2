...
const firstPromise = new Promise((resolve) => 
    //Создаем первый промис, который вернет число
    {
    resolve(2);
});
 
function squareAfterDelay(number) 
//Создаем функцию, в параметр number будет передаваться число, которое нужно будет возвести в квадрат
{
    ...
    //возвращаем новый промис с задержкой выполнения кода на 3 сек.
}
 
firstPromise
    .then((result) => {
        return squareAfterDelay(result);
        //результат передается в функцию squareAfterDelay
    })
    ...
    .then((finalResult) => {
        console.log('Итоговый номер:', finalResult);
    })
